...
public struct Reduce<Input: Collection & Sendable, Output: Sendable>: Step where Input.Element: Sendable {
    public typealias ProcessParameters = (Output, Input.Element, Int)
    /// A closure that produces a step to process each element and accumulate the result
    private let process: @Sendable (ProcessParameters) -> any Step<Output, Output>
    ...
    public init(
        initial: Output,
        @StepBuilder process: @escaping @Sendable (ProcessParameters) -> any Step<Output, Output>
    ) {
